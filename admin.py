...
try:
    ...
    if res.data:
        ...
        with st.sidebar:
            ...
            st.markdown("#### 🔍 BỘ LỌC")
            # Lọc theo Huyện cũ (tương ứng với mstva.py)
            list_huyen = ["Tất cả"] + sorted(df_raw['huyen_cu'].dropna().unique().tolist())
            sel_huyen = st.selectbox("Huyện cũ (HN11):", list_huyen)
            
            df_f = df_raw if sel_huyen == "Tất cả" else df_raw[df_raw['huyen_cu'] == sel_huyen]
        ...
        with c3:
            # Biểu đồ phân bố mini
            chart_data = df_f['huyen_cu'].value_counts().reset_index()
            fig = px.bar(chart_data, x='count', y='huyen_cu', orientation='h', height=150, 
                         color='count', color_continuous_scale='Blues')
            fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), showlegend=False, coloraxis_showscale=False)
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})

        # --- 6. BẢNG DỮ LIỆU (CHỌN DÒNG) ---
        st.info("💡 **Hướng dẫn:** Click chọn (chếch) vào một dòng bên dưới để xem chi tiết và xuất file Excel.")
        
        # Sửa lỗi: Sử dụng 'single-row' thay vì 'single'
        event = st.dataframe(
            df_f, 
        ...
        # --- 7. HIỂN THỊ CHI TIẾT KHI CLICK CHỌN ---
        if len(selected_rows) > 0:
            ...
            # Khai báo các nhãn hiển thị tương ứng với mstva.py
            labels = {
                "mst": "Mã số thuế", "ten_don_vi": "Tên đơn vị", "dia_chi": "Địa chỉ", 
                "huyen_cu": "Khu vực (Huyện cũ)", "ma_qhns": "Mã QHNS", "so_tkkb": "Số TK Kho bạc",
            ...
            with st.container(border=True):
                ...
                for i, (key, label) in enumerate(labels.items()):
                    with grid_cols[i % 3]:
                        val = row.get(key, "Trống")
                        st.markdown(f"**📌 {label}**")
                        st.info(val if val else "Trống")
                
                st.divider()
                
                # Chức năng xuất Excel cho dòng đang chọn
                output = io.BytesIO()
            ...
            st.write("---")
            st.caption("Chưa có đơn vị nào được chọn.")

...
try:
    ...
    with st.sidebar:
        ...
        col_kpi2.metric("Thiếu SĐT", int(missing), delta=f"-{int(missing)}", delta_color="inverse")
        ...
        st.divider()
        sel_h = st.selectbox("Lọc nhanh vùng:", ["Tất cả"] + sorted(df_raw['huyen_cu'].dropna().unique().tolist()))
        st.link_button("🔄 Kiểm tra cập nhật", "https://your-update-link.com", use_container_width=True)
        if st.button("🚪 Đăng xuất", use_container_width=True):
            st.session_state.auth = False
            st.rerun()

    # --- 5. TÌM KIẾM & BẢNG QUẢN TRỊ ---
    st.title("📂 QUẢN LÝ NGHIỆP VỤ KẾ TOÁN HN11")
    search_q = st.text_input("🔍 Tìm kiếm", placeholder="Tên đơn vị, MST, Kế toán, SĐT...")
    ...
    st.write(f"Tìm thấy **{len(df_f)}** đơn vị")
    event = st.dataframe(
        df_f[['mst', 'ten_don_vi', 'huyen_cu', 'ke_toan', 'sdt_ke_toan', 'ma_kbnn', 'san_pham']],
    ...
    # --- 6. CHI TIẾT & CHỈNH SỬA (FIXED: FORM VS DOWNLOAD) ---
    if event.selection.rows:
        ...
        # Phần 1: Form để chỉnh sửa và Lưu
        with st.form("update_form"):
            ...
            save_btn = st.form_submit_button("💾 LƯU THÔNG TIN", type="primary")
        ...
        # Phần 2: Các nút xuất file và liên kết (Nằm ngoài form để tránh lỗi)
        st.write("---")
        st.caption("Các tùy chọn kết xuất cho đơn vị đang chọn:")
